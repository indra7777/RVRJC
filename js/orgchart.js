(function ($) {
	...
	function buildNode($node, $appendTo, level, opts) {
		...
		if ($childNodesCount > 0) {
			...
			$nodeCell.append('<div><img class="cover" src="icons/up.svg"/></div>');
		}
		...
		if ($node.attr('class') != undefined) {
			...
			$.each(classList, function (index, item) {
				if (item == 'collapsed') {
					...
					$nodeRow.find('img.cover').attr('src', 'icons/down.svg');
				} else {
